...
- [ ] O8 GATE HOLE #6: the LITERAL regex needs a capital as the FIRST character, so a run that starts with an arrow/symbol (`← Back`, `▸ Full report`, `◄ LEFT`) is invisible — LineageLauncher's `← Back` was only found by reading the file
