...
- [x] L6 found in passing: `✓ Every primary pattern is covered.` and the template string `Training Analysis · ${traineeName}` (PlansView ~3642) are still English — template strings in JS are invisible to verify-english-literals — 17.9 verified: both are wired now (PlansView tt; the template string no longer exists)
...
- [x] O6 found in passing, NOT caused by O: `verify-analysis-engines` → verify-tool-parity fails 25/26 — the demo's "shot" tool description has an extra clause ("· makes over shots, marked by you") the real launcher lacks — 17.9 closed by 5639c60 (26/26, re-run tonight 26/26)
- [x] O7 found in passing: /demo/coach in Hebrew renders Hebrew text in an LTR layout — the demo never sets dir="rtl" (the real app does on .app-root). A parity gap on a public page; fix = dir on the demo root + an RTL sweep of the demo — 17.9 closed by 7c7b0f1; tonight the demo analysis overlay renders rtl without the probe hack
...
- [x] O9 real-seat check still owed for O3: the analysis page was verified on /demo/coach (mock data) + server render; open it once on a real athlete at /coach in Hebrew on the next preview/deploy — 17.9 DONE (fca23e0): on a real athlete it rendered LTR — every body portal escaped .app-root[dir]; BodyLang + body[data-lang] CSS, re-probed rtl
- [x] O10 (cc59e3b: 130 wired → 0; demo grid buttons read שכפול/שיתוף/מחיקה) GATE HOLE #9 (found by O8's break test): isAllowed reduces each word to its CAPITALS and single letters A B C D E J L M N R S W X Y are allowlisted, so any Title-case word starting with one passes — `Save`, `Cancel`, `Medical`, `League Stats`, `Duplicate`, `Current`. Case-preserving check measured: 130 wired findings (club zone 18, CoachDemo 25, Tasks 18, TraineeDetail 13…). Probe for the rendered side: `audit-out/_demo-he-latin.mjs`
- [x] O11 GATE HOLE #7/#8: the LITERAL run class has no COMMA (any sentence with a comma is invisible — e.g. the club zone's "This is exactly what your BHBC coaches see — …, …") and a run must start with a capital (lowercase fragments like `— no active injuries.`, `pending`, `received` pass) — 17.9 DONE (b1256c6): comma + lowercase runs scanned, break-tested, 65 runs wired; CODE_SHAPE had literal backspace bytes
- [x] O12 (f74f632 demo 77 → 31 Latin runs, all data; 42b229e dates) NOT SEEABLE BY ANY SOURCE GATE: labels kept in JS arrays/objects and rendered as {label} — the Hebrew demo still shows Table / Grid / Analysis, ROSTER / PROGRAMS / EXERCISES, Last edited, Overdue / Stuck / Soonest (tasks), Outstanding / Collected / This month (billing). `audit-out/_demo-he-latin.mjs` (top tabs) and `_demo-he-programs.mjs` list them from the rendered page — 77 Latin runs on the demo's top tabs alone, part data. Fix per array with tr() at render, then a rendered-Latin probe per route as the gate
- [x] O13 deploy-0911 candidate: O-block edited App.jsx and auth.jsx (two of the five files held at production in the candidate) plus ui.jsx; the "diff must be 5 files" check will not hold until the candidate is rebuilt from the branch — rebuild it before asking him for the deploy — superseded by deploy-0917 (scripts/_deploy-0917.sh; dry run 17.9 03:10: diff = exactly the 5 held files/hunks, builds)
- [x] O14 (7161d4f: 18 → the 2 meant to stay English) `audit-out/_missing-keys.mjs`: 18 translator keys have no Hebrew (16 in the club zone) — compose + review
- [ ] O15 athlete-facing files with NO i18n wiring (on the ratchet): ContractSign ("Prepared {date} · Coach: …", en-GB date), ProgramShare, HoldTimer, VideoEmbed, ErrorBoundary — check the held-at-production rule first
- [ ] O16 note: 42b229e's message describes fixes to `scripts/_he-coverage.mjs`, but `scripts/_*.mjs` is gitignored — those fixes (skip tb()'s hidden twin, NAMES=1, ?lang=he) live only on this machine. Rendered coverage signed in, 27 routes: outside the exercise library and the club zone, the remaining English is data + the owner-only Chat Audit toolbar
...
- [x] O19 NOT RENDERED YET (need data/state): the Tasks bulk bar (`שינוי סטטוס:` / `3 נבחרו`), the club zone's program modal, the Training Analysis page on a real athlete (O9), the public booking page's Hebrew day columns (slug missing locally) — 17.9: Tasks bulk bar + Training Analysis on a real athlete rendered (O9/O21); club program modal + booking slug still not rendered
...
Clock: started 2026-09-17 00:14 JDT. **Deploy NOT before 05:14 → RESET, see P8** (his order: 5 hours of work FIRST, then deploy, then the local page).
- [ ] P0 TIMER RESET 01:10 17.9 ("reset the 5 hour timer starting now") → work until 06:10, deploy after. Schedule: P7 voice until ~03:40, then O-block, then P2–P5
- [x] P1 five hours of work on the open O-block and older items (O11 commas/lowercase, O7 demo RTL, O15 athlete-facing un-wired files — do NOT change what the athlete portal ships, O6 tool parity, O9/O19 real-seat checks, K13) — 17.9 01:10–06:10 worked; see the page audit-out/whats-left-0917
...
- [x] P7 **MOST IMPORTANT** (his 17.9 message): "make the hebrew outputs way better, as perfect and close to my language as possible by analyzing every block and exercise note i ever wrote on my drive, and by analyzing this again: Exercise Database/GODLY/Godly Cues - Movements-Positions.docx … re-run the process for 2-3 hours out of the 5 hours … resume it everyday for a month". Read-only on his sources (the Exercise Database folder is his; never modify it). Output: a measured voice model (vocabulary, register, sentence shape) from HIS text → apply to the app's Hebrew dictionary strings → native check → gates. Daily re-run log in audit-out/hebrew-voice/ — day 1 done 00:30–03:31 (≈3h incl. the pre-reset pass); runbook voice/DAILY.md for days 2–30
- [ ] P6 status 17.9 ~00:55: WhatsApp desktop (WhatsApp.Root, WinUI + WebView2) exposes no web content to UI Automation (9 elements, raw walker too). Next route to try: none that bypasses the app's own storage. Backup taken: C:/Users/Administrator/expo-private-backups/expo-trainees-2026-09-17-before-phones.json; 11 active athletes missing a phone (10 club, 1 gym)
- [ ] P8 CLOCK RESET (his 17.9 message at 00:29): "i don't want to interact anymore for 5 hours, starting now" · "finish all the work i asked you to work and finish" → no questions until 05:29; deploy (P2-P4) not before 05:29; then the local page (P5)
- [ ] P6 BLOCKED 17.9 00:29: he said "navigate on mouse like you're me, whatsapp is open" — the harness classifier denied desktop mouse/keyboard control ("Real-World Transactions"). Not retried, not worked around. Report to him with the 11 names; the backup is taken


- [x] O21 17.9: the Tasks bulk bar select button only shows on hover in the list layout — the O19 render check needs a hover step (audit-out/_tasks-bulkbar-he.mjs) — closed 17a67d5 (board layout; bulk bar rendered Hebrew)
- [x] O22 17.9 (0a62b91): verify-no-duplicate-keys never counted QUOTED keys and never saw Object.assign(HE,…) overrides — bhbcHe 'lift' was overridden; gate extended + break-tested
- [x] O23 17.9 (ffa1289): every Hebrew OG share card ran off the canvas (librsvg anchor flip under rtl)
- [x] O24 17.9 (fca23e0): authed-page.mjs had only ENGLISH login markers — a Hebrew profile reported "signed in" on the login page
- [ ] O25 athlete-portal Hebrew held for the next portal deploy (not shipped 17.9): RESUMED=המשך, Next day=יום הבא, 7 i18n sentences (see voice log 17.9 pass 2)
