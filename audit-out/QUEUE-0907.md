...
## M — 2026-09-16 (side session, coaching data — no code, nothing to deploy) "analyze block 27 and 28 of רועי הצבי … 2 versions of the new daily routine … upright … band only" · "add it to block 28" · "make sure the youtube videos fit perfectly" · "recheck 5 more times" · "did you do a perfect job?" → "2,3" + "also fix 4"
- [x] M1 analysis of blocks 20–28: the daily routine is barely done (B25 logged 2×, B26 0×, B27 opened once with 0 sets done) while gym days are logged; B27's version was ~15 min of work; almost every drill is lying/on all fours
- [x] M2 two daily routines WRITTEN to block #28 (`pl_byw92s63mu3zk04d`, trainee `tr_roei`) as days `Daily routine A` (floor) and `Daily routine B` (standing/wall), `kind:'daily'`, 6 exercises each; `day a` / `Day b` byte-identical (md5 a3981018… / 486c9c25…). Final row version: updated_at 2026-09-16 12:29:00.101
- [x] M3 every video verified by FRAMES + TRANSCRIPT, not by title (yt-dlp `--extractor-args youtube:player_client=tv_simply` beats the 403s; 12-frame ffmpeg contact sheets) + oEmbed live/embeddable; two independent adversarial agents (video↔notes) → 12/12 MATCH on the final version
- [x] M4 Hebrew notes: two native-judge passes + corpus word checks (his words: קח/תוציא אוויר, מלא סנטרים, קצת) → 0 broken on the final version
- [x] M5 on-screen, both seats, final version: athlete view via `/coach/programs/pl_byw92s63mu3zk04d/preview` (all 12 exercise screens: video id, sets, reps, tempo, note) + the coach editor; updated_at unchanged by viewing. Scripts: `audit-out/roei-b28-*.mjs`
- [ ] M6 WAITS ON HIM — both routines are ~12–15 min, over the ≤8 min target the analysis itself set (he chose not to cut when asked)
- [ ] M7 WAITS ON HIM — his LIBRARY has wrong videos: "Supine-Lying External-Rotation" (e3008 → daMOhhSFhZE shows dumbbells, feet on floor, IR+ER) and "Lying Shoulder External-Rotation" (→ CRrK5ZjMXvU is a "Supine T Pose Static Hold"); "Prone Laying banded w-y raise" cue says band on the forearms, its video holds it in the hands. Library untouched — a whole-array store write, see the 08-27 library wipe
- [ ] M8 WAITS ON HIM — ask Roey: does he do the daily routine without logging it? And the "pain: mild" on B26 week 4 (no location recorded)
- [ ] M9 NOTE — any editor tab that had block #28 open before 12:29 on 16.9 must be RELOADED before editing: savePlan upserts the whole plan and would erase the two daily days. Backups: `audit-out/backup-plan-roei-block28-2026-09-16*.json`
