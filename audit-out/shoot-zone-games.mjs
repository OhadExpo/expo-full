...
// games tab
const names = await pg.evaluate(() => [...document.querySelectorAll('.bhbc-tab,[role="tab"]')].map((x) => (x.textContent || '').trim()));
console.log('tabs:', JSON.stringify(names));
const ok = await pg.evaluate(() => { const el = [...document.querySelectorAll('.bhbc-tab,[role="tab"]')].find((x) => /^(ליגה|משחקים|League|Games)$/.test((x.textContent || '').trim())); if (!el) return false; el.click(); return true; });
await wait(4000);
await pg.screenshot({ path: `audit-out/${TAG}-games.png`, fullPage: true });
console.log('games tab:', ok);
await pg.close();
b.disconnect();
