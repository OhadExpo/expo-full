...
const looksRight = await pg.evaluate(() =>
  document.querySelectorAll('a[href*="#/programs/"]').length > 0
  || /expo-il/i.test(document.body.innerHTML));
if (!looksRight) {
  console.log('FAILED: ' + BASE + ' is not serving the marketing site.');
  console.log('Start it from inside expo-il:  cd expo-il && npm run dev -- --port 5174 --host 127.0.0.1');
