...
- ~~Shot analyzer returns a different shot count on repeat runs — 11, 10, 9~~ — the cause is confirmed (capture samples a playing video and loses frames) and there is now a gate: `scripts/verify-shot-determinism.mjs` asserts N runs agree on count AND per-shot release times. It is only meaningful with `deterministic: 'coarse'`; the default capture still varies, so any before/after claim about the analyzer MUST use it. Still true: do NOT tune detection thresholds against a varying count.
