...
- ~~Exercise Library Batch Matching Screen — resolve unmatched exercise titles at scale~~ — SHIPPED. `src/ExerciseMatchingView.jsx` (311 lines) is live at `/coach/exercise-matching` (Athletes▾ → Matching), listed in `docs/SURFACES.md` since 2026-08-16, and console-clean in the 2026-08-27 route sweep. Verified 2026-08-27 — it had still been listed as "next priority", which misdirects every session that reads this file.
...
- ~~Longer-term: MediaPipe Pose Landmarker integration for exercise form video analysis~~ — SHIPPED and well past this description. `src/usePose.js` wraps `@mediapipe/tasks-vision`; `poseLab.analyzeClip` does rep segmentation, ROM/tempo, bar velocity and per-joint angles; it is live in `MovementLab` at `/coach/review-tools`, and the Shot Analyzer is a second pipeline on the same landmarker. Verified 2026-08-27.
- ~~Shot analyzer returns a different shot count on repeat runs — 11, 10, 9~~ — the cause is confirmed (capture samples a playing video and loses frames) and there is now a gate: `scripts/verify-shot-determinism.mjs` asserts N runs agree on count AND per-shot release times. It is only meaningful with `deterministic: 'coarse'`, which buys REPEATABILITY, not completeness — measured 2026-09-01 on Ohad's own 17-shot clip, deterministic captured 1541 frames and found 9 shots in 1445s while the default captured 2286 and found all 17 in 487s (60fps portrait makes seeks expensive and the fixed step under-samples). Use it to prove a change added no VARIANCE; never as evidence a shot count is right. Still true: do NOT tune detection thresholds against a varying count.
- **Actually next** (2026-09-01, measured): the shot COUNT is repeatable and the BALL TRACK is not. Three controlled runs of his own 11-shot clip returned 11 / 11 / 11 shots (spread 0) but 0 / 10 / 10 launch angles — one run in three produced no angle at all. So the earlier "11, 10, 9" framing is out of date: do NOT chase detection thresholds. `scripts/shot-stability.mjs <clip> <runs> <rates...>` reports the spread per coarse playback rate; `shotCapture.js` already proves the frames are lost upstream (its busy-skip counter reads zero — the browser presents fewer frames while MediaPipe runs), and seek-stepping every frame was tried and measured WORSE (9 of 17 shots, 3× slower). **`coarseRate` was the untried lever and it has now been tried — it does NOT help (measured 2026-09-04, n=2 per rate, on his 11-shot clip, on a browser with no leftover tabs):** `rate 1` → 11/11 shots, **9 and 9** angles, 330s avg. `rate 0.5` → 11/11 shots, **10 and 9** angles, 365s avg. Halving the rate hit a higher maximum ONCE but was less consistent than rate 1 and cost 11% more time, so do not change it on this evidence. **More important: the run-to-run angle instability did not reproduce.** Rate 1 returned the same 9 angles both times, against the "0 / 10 / 10" recorded on 09-01. Before blaming the tracker again, count the open tabs — `curl -s --noproxy '*' http://127.0.0.1:9222/json/list | grep -c '"type": "page"'`. Scripts that connect to the debug Chrome and exit without closing their page had left 32 pages open, and this analysis is timing-sensitive. Launch angle on footage where the release is above the frame stays unmeasurable — run `scripts/verify-clip-usable.mjs` before touching the tracker; see `docs/ball-launch-diagnosis-2026-08-31.md`. Residual subject swaps: 11 across the ten-clip corpus, in c01 and c09 (`scripts/verify-shooter-lock.mjs`).
