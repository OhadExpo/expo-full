...
- **Actually next** (2026-08-31): nothing outstanding on the ball launch angle — it is unmeasurable on footage where the release happens above the frame, which is the case on Ohad's own clip. `scripts/verify-clip-usable.mjs` says whether a clip can answer the question at all; run it before touching the tracker. See `docs/ball-launch-diagnosis-2026-08-31.md`. The open shot-analyzer work is the residual subject swaps: 11 across the ten-clip corpus, concentrated in c01 and c09 (`scripts/verify-shooter-lock.mjs`).
