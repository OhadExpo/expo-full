...
- **Actually next** (2026-08-27): the shot analyzer returns a DIFFERENT shot count on repeat runs of the same clip — 11, 10, 9 — because capture samples a playing video and loses frames under load. The count is the first number a coach reads. See `docs/shot-analyzer-next-2026-08-27.md`; an opt-in deterministic capture mode exists to measure the fix. Do NOT tune detection thresholds against it.
