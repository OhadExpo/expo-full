...
The harness reported them honestly as ERROR — my grep pattern was the thing that
missed them, not the tool.

Re-measured at 05:01 on a quiet machine, so this is evidence and not a
prediction:

```
ok  /coach/waitlist   sw=390 bsw=390 iw=390
ok  /coach/workouts   sw=390 bsw=390 iw=390
ok  /coaches/try      sw=390 bsw=390 iw=390
```

All three fit. 37/37 stands, and all 37 are now measured.
