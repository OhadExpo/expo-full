// Floating chat assistant for expo-il (athlete-side). Hits the same
// Anthropic-proxy pattern as the coach-side widget but wired to this
// project's /api/chat with an athlete-scoped system prompt. Soft email
...
export default function Chat() {
  ...
  // Re-detect lang every time the panel opens — visitors toggle EN/HE on this site.
  useEffect(() => { if (open) setLang(detectLang()); }, [open]);

  const isHe = lang === 'he';
  const SUGGESTIONS = isHe ? SUGGESTIONS_HE : SUGGESTIONS_EN;
  const CAPTURE_PROMPT = isHe ? CAPTURE_PROMPT_HE : CAPTURE_PROMPT_EN;
  ...
  }, [messages, capturePrompted, CAPTURE_PROMPT]);
  ...
  const send = async (text) => {
    ...
    if (!t || sending || healthy === false) return;
    setErr('');
  ...
  // Anchor on the start side based on doc direction so the bubble doesn't
  ...
  return (
    <>
      ...
      {open && (
        <div role="dialog" aria-label="EXPO chat" dir={isHe ? 'rtl' : 'ltr'}
          style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: 1, height: 32 }}>
              ...
              <div style={{ fontSize: 12, color: C.tm, lineHeight: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center' }}>
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: 12, color: C.tx, lineHeight: 1.5, marginBottom: captureState === 'done' ? 0 : 8 }}>
                      {captureState === 'done'
                        ? (isHe ? '✓ הפרטים נשלחו. אוהד יחזור אליך בקרוב.' : "✓ Got it. Ohad will reach out soon.")
                        : m.content}
                    </div>
                    {captureState !== 'done' && (
