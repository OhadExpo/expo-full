...
export const INTAKE_HE = {
  ...
  intro: 'מלא את הטופס פעם אחת. התשובות יגיעו ישר למאמן ויעזרו להתאים לך את התוכנית.',
  submitLabel: 'שליחה',
  thanksTitle: 'תודה רבה!',
  thanksBody: 'הטופס הגיע למאמן. נחזור אליך בקרוב.',
  questions: [
    ...
    { id: 'relationship',  type: 'choice',    label: 'מצב משפחתי', required: true, choices: ['רווק/ה', 'נשוי/אה', 'בזוגיות'] },
    { id: 'gym_first',     type: 'choice',    label: 'זו הפעם הראשונה שלך בחדר כושר?', required: true, choices: ['כן (אני חדש לגמרי)', 'לא (התאמנתי בעבר)'] },
    { id: 'gym_longest',   type: 'short',     label: 'אם התאמנת בעבר — בערך כמה זמן התאמנת ברצף בתקופה הכי ארוכה?' },
    { id: 'gym_last_date', type: 'short',     label: 'אם התאמנת בעבר — מתי היה האימון האחרון?' },
    { id: 'gym_stop_why',  type: 'paragraph', label: 'אם הפסקת בעבר — מה הסיבה?' },
    { id: 'weight_history',type: 'paragraph', label: 'היית פעם במשקל גבוה מאוד או רזה מאוד? פרט.', required: true },
    { id: 'injuries',      type: 'paragraph', label: 'נפצעת פעם? פרט.', required: true },
    { id: 'physical_state',type: 'paragraph', label: 'מה המצב הגופני שלך היום? (כאבים, טווחי תנועה, חולשות, כל דבר אחר)', required: true },
    { id: 'medical',       type: 'paragraph', label: 'פרטים רפואיים או מחלות כרוניות שחשוב לדעת עליהם?' },
    ...
    { id: 'sleep_quality', type: 'scale',     label: 'דרג את איכות השינה הממוצעת', required: true, scale: { min: 1, max: 5, minLabel: 'שינה לא יציבה', maxLabel: 'שינה עמוקה ורציפה' } },
    { id: 'stress_work',   type: 'scale',     label: 'דרג את רמת הלחץ בעבודה / עיסוק', required: true, scale: { min: 1, max: 5, minLabel: 'לא מציק לי', maxLabel: 'מרגיש לחץ כל הזמן' } },
    { id: 'stress_family', type: 'scale',     label: 'דרג את רמת הלחץ במצב המשפחתי / זוגי', required: true, scale: { min: 1, max: 5, minLabel: 'לא מציק לי', maxLabel: 'מרגיש לחץ כל הזמן' } },
    { id: 'stress_general',type: 'choice',    label: 'דרג את רמת הלחץ הכללי בחיים', required: true, choices: ['לא לחוץ בדרך כלל', 'יותר לחץ לאחרונה', 'לחץ לעיתים קרובות', 'לחץ קיצוני בחיים'] },
    { id: 'main_goal',     type: 'paragraph', label: 'מה המטרה העיקרית שלך באימונים איתי?', required: true },
    { id: 'extra_goals',   type: 'paragraph', label: 'מטרות נוספות?' },
    { id: 'avoid',         type: 'paragraph', label: 'דברים שתרצה להימנע מהם?' },
    { id: 'lifestyle',     type: 'paragraph', label: 'הרגלים חדשים או שינויים באורח החיים שאתה רוצה לעבוד עליהם?' },
...
export const PROGRESS_HE = {
  ...
  intro: 'הערכת ביניים. המטרה: לעדכן אותי איך שגרת האימונים מתאימה לך עכשיו.',
  submitLabel: 'שליחה',
  thanksTitle: 'תודה!',
  thanksBody: 'קיבלתי את העדכון. אם משהו דחוף — שלח לי הודעה.',
  questions: [
    { id: 'email',           type: 'email',     label: 'אימייל', required: true },
    { id: 'name',            type: 'short',     label: 'שם מלא', required: true },
    { id: 'weight',          type: 'number',    label: 'משקל גוף נוכחי (ק"ג)', required: true },
    { id: 'composition',     type: 'choice',    label: 'היו שינויים בהרכב הגוף?', required: true, choices: ['לא, לא שמתי לב לשינוי', 'אחר'] },
    { id: 'composition_note',type: 'paragraph', label: 'אם "אחר" — פרט' },
    { id: 'medical',         type: 'paragraph', label: 'פרטים רפואיים או מחלות כרוניות שחשוב לדעת עליהם?' },
    { id: 'allergies',       type: 'paragraph', label: 'אלרגיות? רגישויות?' },
    { id: 'relationship',    type: 'choice',    label: 'מצב משפחתי', choices: ['רווק/ה', 'נשוי/אה', 'בזוגיות'] },
    { id: 'sleep_hours',     type: 'choice',    label: 'ממוצע שעות שינה בלילה בשבועות האחרונים', required: true, choices: ['בין 4 ל-5', 'בין 5 ל-6', 'בין 6 ל-7', 'בין 7 ל-8'] },
    { id: 'sleep_quality',   type: 'scale',     label: 'דרג את איכות השינה הממוצעת', required: true, scale: { min: 1, max: 5, minLabel: 'שינה לא יציבה', maxLabel: 'שינה עמוקה ורציפה' } },
    { id: 'stress_family',   type: 'scale',     label: 'דרג את רמת הלחץ במצב המשפחתי / זוגי', required: true, scale: { min: 1, max: 5, minLabel: 'לא מציק לי', maxLabel: 'מרגיש לחץ כל הזמן' } },
    { id: 'stress_work',     type: 'scale',     label: 'דרג את רמת הלחץ בעבודה / עיסוק', required: true, scale: { min: 1, max: 5, minLabel: 'לא מציק לי', maxLabel: 'מרגיש לחץ כל הזמן' } },
    { id: 'stress_general',  type: 'choice',    label: 'דרג את רמת הלחץ הכללי בחיים', required: true, choices: ['לחוץ לעיתים רחוקות', 'לחוץ מדי פעם', 'לחוץ לעיתים קרובות', 'לחוץ מאוד בתקופה הנוכחית'] },
    { id: 'training_impact', type: 'choice',    label: 'שגרת האימונים השפיעה על היום-יום שלך?', required: true, choices: ['לא שמתי לב', 'אחר'] },
    { id: 'training_note',   type: 'paragraph', label: 'אם משהו החמיר או השתפר — פרט/י' },
    { id: 'physical_state',  type: 'paragraph', label: 'מה המצב הגופני שלך היום?' },
    { id: 'new_pain',        type: 'paragraph', label: 'יש כאבים חדשים?' },
    { id: 'rom_limit',       type: 'paragraph', label: 'יש מקום שבו טווח התנועה מוגבל?' },
    { id: 'other_issue',     type: 'short',     label: 'משהו אחר שמציק / כואב / מפריע?', required: true },
    { id: 'satisfaction',    type: 'scale',     label: 'מה רמת הסיפוק וההנאה שלך מהאימונים?', required: true, scale: { min: 1, max: 5, minLabel: 'נמוכה מאוד', maxLabel: 'גבוהה מאוד' } },
    { id: 'system_feedback', type: 'choice',    label: 'הצעות לשיפור במערכת ובכלים?', required: true, choices: ['לא, אני בסך הכל מרוצה', 'אחר'] },
    { id: 'system_note',     type: 'paragraph', label: 'אם "אחר" — פרט' },
...
export const ASSESSMENT_HE = {
  ...
  intro: 'ממלאים פעם אחת. הנתונים האלה הם נקודת הפתיחה שממנה נבנה את הבלוק הראשון. בדיקות עצמיות פשוטות — לא צריך סרט מדידה או מד-זווית.',
  submitLabel: 'שליחה',
  thanksTitle: 'תודה!',
  thanksBody: 'קיבלנו את ההערכה. נחזור אליך עם תוכנית ראשונה.',
  questions: [
    { id: 'email',           type: 'email',     label: 'אימייל', required: true },
    { id: 'name',            type: 'short',     label: 'שם מלא', required: true },
    { id: 'assessment_date', type: 'date',      label: 'תאריך ההערכה', required: true },
    // --- Functional tests
    { id: 'sit_rise',        type: 'scale',     label: 'מבחן ישיבה-קימה (0–10): קום מישיבה על הרצפה לעמידה בלי להיעזר בידיים. תוריד נקודה על כל יד/ברך שנגעה ברצפה.', required: true, scale: { min: 0, max: 10, minLabel: 'צריך עזרה', maxLabel: 'בלי תמיכה' } },
    { id: 'balance_left',    type: 'number',    label: 'עמידה על רגל שמאל (שניות, עיניים פקוחות — עד 60)', required: true },
    { id: 'balance_right',   type: 'number',    label: 'עמידה על רגל ימין (שניות, עיניים פקוחות — עד 60)', required: true },
    { id: 'pushup_max',      type: 'number',    label: 'שכיבות סמיכה רצופות עד כישלון (אפס אם לא יוצאת אף אחת)', required: true },
    { id: 'plank_seconds',   type: 'number',    label: 'פלאנק על האמות — זמן בשניות עד שהאגן יורד', required: true },
    ...
    // --- Pain map
    { id: 'pain_regions',    type: 'multichoice', label: 'אזורים בגוף שכואבים או מגבילים תנועה כרגע (סמן את כל מה שרלוונטי)', required: true, choices: PAIN_REGIONS_HE },
    { id: 'pain_worst',      type: 'scale',     label: 'דרג את הכאב הכי חזק כרגע (0 — בלי כאב, 10 — בלתי נסבל)', required: true, scale: { min: 0, max: 10, minLabel: 'בלי כאב', maxLabel: 'בלתי נסבל' } },
    { id: 'pain_detail',     type: 'paragraph', label: 'תאר את הכאב הכי משמעותי — מתי הוא מופיע, מה מחמיר אותו, מה מקל' },
    // --- History
    ...
    { id: 'medications',     type: 'paragraph', label: 'תרופות שמשפיעות על אימון (מדללי דם, סטרואידים, חוסמי בטא וכו\')' },
