    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      {arr.map((em, i) => (
        <div key={i} style={{ display: 'flex', gap: 4 }}>
          ...
          {arr.length > 1 && <button onClick={() => { const next = [...arr]; next.splice(i, 1); onChange(next); }} style={{ background: 'transparent', border: `0.25px solid ${C.rd}`, borderRadius: 0, padding: '0 10px', color: C.rd, cursor: 'pointer', fontSize: 14, lineHeight: 1 }}>×</button>}
        </div>
      ))}
      {arr.length < max && (
  ...
  return (
    <div style={{ position: 'fixed', left: '50%', bottom: 20, transform: 'translateX(-50%)', zIndex: 1300, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', pointerEvents: 'none', maxWidth: 'calc(100vw - 32px)' }}>
      {items.map(it => {
        ...
        return (
          <div key={it.id} style={{ pointerEvents: 'auto', background: C.bg, color: p.fg, border: `1px solid ${p.bd}`, borderRadius: 0, padding: '12px 16px', fontFamily: FB, fontSize: 13, fontWeight: 500, boxShadow: '0 8px 24px rgba(0,0,0,0.6)', minWidth: 240, maxWidth: 480, display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'center' }}>
