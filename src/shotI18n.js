...
export const SHOT_I18N = {
  ...
  he: {
    ...
    ballFlat: 'המסלול שעקבנו אחריו כמעט לא עולה, אז זו לא יכולה להיות הזריקה — הזווית יצאה שגויה, ולכן היא לא מוצגת.',
    ...
    checks: {
      dip: {
        ...
        why: 'הרגליים הן המנוע. דיפ רדוד מדי מכריח את היד לייצר את כל הכוח — זריקה שטוחה שיוצאת רק מהידיים ולא מגיעה מרחוק. דיפ עמוק מדי מאט את הקצב ונותן להגנה להתקרב. כיפוף ברך בינוני שומר גם על דחיפת רגליים וגם על קצב.',
      ...
      setHeight: {
        ...
        how: ['זריקות פורמה לקיר: עמוד 30 ס״מ מהקיר, תביא את הכדור למצח ותתמתח ישר למעלה בלי שהמרפק ייגע בקיר.', 'זריקה עם עצירה: תחזיק את הסט שנייה, בדוק ששורש כף היד בגובה הגבה, ואז שחרר.', 'דימוי: "כדור למצח, לא לסנטר".'],
      },
      setElbow: {
        label: 'מרפק בסט',
      ...
      elbowAlign: {
        ...
        how: ['תרגיל החלקה בקיר: כתף יד הזריקה לקיר, המרפק מלטף את הקיר לאורך כל היישור.', 'דימוי: "מרפק לחישוק" — הוא מצביע על המטרה לפני שהיד יוצאת.', 'שליטה ביד התומכת: היד השנייה נשארת בצד הכדור עד השחרור, ואז עוזבת אותו בלי לדחוף.'],
      },
      releaseExt: {
        ...
        how: ['"תיכנס עם היד לחישוק" — האצבעות מסיימות מכוונות לטבעת, המרפק נעול.', 'זריקות מכיסא: זריקות פורמה בישיבה, כך שהכוח חייב להגיע מהיישור ומשורש כף היד ולא מהרגליים.', 'חזרות צל ב-50% מהמהירות — תחזיק את הסיום המיושר 2 שניות.'],
      },
      releaseArm: {
        label: 'זווית היד בשחרור',
        target: null, // built from the shot-type band in localiseCheck()
        why: 'זווית האמה בשחרור קובעת את זווית השיגור של הכדור. שטוחה מדי — חלון הכניסה לחישוק מצטמצם. תלולה מדי — מפסידים מרחק ותזמון. זו זווית היד. בשביל זווית הכדור האמיתית צריך מעקב אחרי הכדור.',
