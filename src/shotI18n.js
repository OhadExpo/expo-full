...
export const SHOT_I18N = {
  ...
  he: {
    ...
    ballFlat: 'המסלול שנעקב כמעט לא עולה, אז זו לא יכולה להיות הזריקה — הזווית יצאה שגויה, ולכן היא לא מוצגת.',
    ...
    sessionWander: 'לא עקבי — חזור, אל תשנה',
    ...
    oblique: 'הזריקה הזאת צולמה בזווית — הכדור התרחק מהמצלמה במקום לנוע לרוחב. הזווית וההשוואה בין החזרות עדיין טובות. המטרים והמ׳/ש׳ יוצאים נמוכים. כדי למדוד אותם נכון, צלם מול הזריקה.',
    ...
    checks: {
      ...
      setHeight: {
        ...
        how: ['זריקות פורמה לקיר: עמוד 30 ס״מ מהקיר, תביא את הכדור למצח והתמתח ישר למעלה בלי שהמרפק ייגע בקיר.', 'זריקה עם עצירה: תחזיק את הסט שנייה, בדוק ששורש כף היד בגובה הגבה, ואז שחרר.', 'דימוי: "כדור למצח, לא לסנטר".'],
      },
      setElbow: {
        label: 'מרפק בסט',
        ...
        how: ['חזרות מול מראה: תעמיס את הכדור בסט וחפש את ה-L — אמה אנכית, זרוע עליונה מקבילה לרצפה.', 'זריקות פורמה ביד אחת, יד מכוונת מחוץ לכדור, מ-2 מ׳, 20 חזרות.'],
      },
      elbowAlign: {
      ...
      releaseExt: {
        ...
        why: 'יישור מלא נותן את המנוף הארוך ביותר ואת נקודת השחרור הגבוהה ביותר, ומאפשר לשורש כף היד לפרוק כחוליה האחרונה בשרשרת. יד קצרה (מתחת ל-145 מעלות) דוחפת את הכדור מהכתף — קשת נמוכה ומרחק לא עקבי.',
      ...
      releaseArm: {
        ...
        why: 'זווית האמה בשחרור קובעת את זווית השיגור של הכדור. שטוח מדי — חלון הכניסה לחישוק מצטמצם. תלול מדי — מפסידים מרחק ותזמון. זו זווית היד. זווית הכדור האמיתית דורשת מעקב אחרי הכדור.',
        how: ['תרגיל קשת: זרוק מעל מטרה 30–40 ס״מ מעל החישוק (יד של שותף על ארגז) — רק סוויש.', 'דימוי: "זרוק למעלה, לא לכיוון" — כוון לנקודה הגבוהה של הקשת, לא לחישוק.', 'צלם מהצד: אצבעות הליווי מסיימות גבוה, לא מצביעות שטוח על החישוק.'],
      },
      timing: {
      ...
      sequence: {
        ...
        how: ['חזרות איטיות "למטה-למעלה-דרך": תרגיש שהרגליים מסיימות לפני שהיד יוצאת.', 'זריקות פורמה בתנועה אחת קרוב לחישוק, עם נסיגה הדרגתית — אותו סדר בכל מרחק.', 'דימוי: "דחוף את הרצפה, ואז את הכדור".'],
      },
      follow: {
        label: 'החזקת הליווי',
      ...
      trunk: {
        ...
        how: ['רגליים קודם: תנחת בג׳אמפ-סטופ עם רגליים מסודרות וחזה זקוף לפני שהכדור מגיע.', 'זריקות בכריעה גבוהה (טול-נילינג), 10 חזרות — הגו לא יכול לנטות.', 'החזקות RDL על רגל אחת 3×20 שניות לשיווי המשקל שמתחת.'],
      },
    },
  },
};
