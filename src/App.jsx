...
function AuthedApp() {
  ...
  // The 4th slot is loadError, and until now NOTHING in the app read it: the
  // store falls back to its localStorage snapshot on a failed read, so a physio
  ...
  // Who works in the club zone and what they touch (owner-only tab inside it).
  const [bhbcActivity,setBhbcActivity]=useSupaStore('expo-bhbc-activity',[]);
          ...
          <nav ref={coachNavRef} className="hdr-scroll" style={{display:"flex",gap:6,alignItems:"center",flex:"1 1 auto",justifyContent:"center",minWidth:0,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
            ...
            {visibleTabs.map(t=>{
              ...
              const activeStyle=isChosen
                ?{background:isActive?CYAN:'transparent',border:'1px solid transparent',color:isActive?'#FFFFFF':BLACK,boxShadow:isActive?'0 1px 2px rgba(0,0,0,0.10), 0 4px 12px rgba(57,189,255,0.28)':'none'}
