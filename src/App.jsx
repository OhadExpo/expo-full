    ...
    <div style={{background:C.bg,color:C.tx,minHeight:"100vh",fontFamily:FB}}>
      <header style={{background:C.headerBg,borderBottom:`1px solid ${C.cardBd}`,boxShadow:'0 1px 2px rgba(0,0,0,0.03), 0 4px 12px rgba(0,0,0,0.04)',position:"sticky",top:0,zIndex:100}}>
        <style>{`
          ...
          .nav-item-inactive:hover{color:#39BDFF !important;background:rgba(57,189,255,0.06) !important}
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"12px"}}>
        <Suspense fallback={<ViewFallback />}>
          {tab==="dashboard"&&<DashboardView trainees={trainees} planCounts={planCounts} workouts={workouts} clientWorkouts={clientWorkouts} payments={payments} presence={presence} onSelectTrainee={id=>navTo("trainees",id)} onOpenTasksTab={()=>navTo("tasks")} onCreatePlanForTask={()=>navTo("plans")}/>}
          ...
          {tab==="tasks"&&<CoachTasksView trainees={trainees} onSelectTrainee={id=>navTo("trainees",id)} onCreatePlanForTask={()=>navTo("plans")}/>}
        </Suspense>
      </main>
