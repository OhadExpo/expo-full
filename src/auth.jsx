      ...
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={cardStyle} dir={he ? 'rtl' : 'ltr'}>
          ...
          <input
            ...
            type="email"
            autoComplete="email"
            style={{ width: '100%', background: 'var(--c-sf)', border: `1px solid ${error ? C.rd : C.cardBd}`, borderRadius: 0, padding: '0 14px', color: C.tx, fontFamily: FB, fontSize: 14, outline: 'none', boxSizing: 'border-box', marginBottom: 8, textAlign: 'center' }}
          />
          <input
            value={password}
            ...
            style={{ width: '100%', background: 'var(--c-sf)', border: `1px solid ${error ? C.rd : C.cardBd}`, borderRadius: 0, padding: '0 14px', color: C.tx, fontFamily: FB, fontSize: 14, outline: 'none', boxSizing: 'border-box', marginBottom: 10, textAlign: 'center' }}
          />
    ...
    <div style={{ position: 'fixed', bottom: 20, right: 20, display: 'flex', flexDirection: 'column', gap: 8, zIndex: 2000, maxWidth: 360 }}>
      {errors.map(e => (
        <div key={e.id} style={{ background: C.bg, border: `1px solid ${C.rd || '#c94444'}`, color: C.rd || '#ff6b6b', borderRadius: 0, padding: '12px 14px', fontFamily: FB, fontSize: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
          ...
          <div style={{ color: C.tx, fontSize: 12 }}>{tr(readLang(), e.msg)}</div>
          ...
          <div style={{ color: C.tm, fontSize: 10, marginTop: 4 }}>{tr(readLang(), e.key === 'form_video' ? 'This recording was not saved — record and upload it again.' : 'Your data is still in local memory. Check connection and retry.')}</div>
        </div>
      ))}
    </div>
  );
