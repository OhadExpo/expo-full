...
function StepLogger({day, plan, weekNum, clientId, onBack, onComplete, weeklyFocus}) {
  ...
  if (typeof step === 'string' && step.startsWith('wu')) {
    ...
    return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>{bar}
      <div style={{padding:20}}>
        ...
        {!vid && <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:30,marginBottom:14,textAlign:'center',color:C.td}}>No video for this exercise</div>}
  ...
  const renderExerciseBlock = (g, blockIdx) => {
    ...
    return <div key={ei} style={{marginBottom: blockIdx < groupExs.length - 1 ? 24 : 0, paddingBottom: blockIdx < groupExs.length - 1 ? 20 : 0, borderBottom: blockIdx < groupExs.length - 1 ? `2px dashed ${C.bd2}` : 'none'}}>
      ...
      <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:14}}>
      ...
      <div style={{background:C.sf,border:`2px solid ${f.uploaded?C.gn+'60':C.ac}`,borderRadius:12,padding:14}}>
  ...
  if (vw === 'bwt' && trainee) {
    ...
    return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>
      ...
      <div style={{padding:'14px 20px 20px'}}>
        ...
        {/* Quick log */}
        <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:16}}>
        ...
        {/* Graph */}
        {bwData.length < 2 ? (
          <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:40,textAlign:'center',color:C.td,marginBottom:16}}>
      ...
      {bwDeleteConfirm && <div onClick={() => setBwDeleteConfirm(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:20}}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:20,maxWidth:320,width:'100%'}}>
  ...
  if (vw === 'hist' && trainee) return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>
    ...
    <div style={{padding:'14px 20px 20px'}}>
      ...
      {cw.length === 0 ? <div style={{textAlign:'center',padding:40,color:C.td}}>No workouts yet.</div> :
        cw.slice().reverse().map(w => <div key={w.id} style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:10,padding:12,marginBottom:8}}>
          ...
          {w.exercises.map((x,i) => {
            ...
            return (
              <div key={i} style={{marginTop:2}}>
                ...
                {isOpen && hasVideo && (
                  <div style={{marginTop:6,marginBottom:10,background:C.sf2,border:`2px solid ${C.ac}`,borderRadius:8,padding:8}}>
  ...
  if (trainee) { const lb = bwLog.filter(b => b.clientId === ci).slice(-1)[0]?.bw;
    return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>
      ...
      <div style={{padding:'14px 20px 20px'}}>
        ...
        {activePlan?.rest && <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:10,padding:'10px 14px',marginBottom:14,fontSize:12,color:C.tm}}>⏱ {activePlan.rest}</div>}
        ...
        {visPlans.length===0 && !plansLoadError && <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:30,textAlign:'center',color:C.td,marginBottom:14}}><div style={{fontSize:20,marginBottom:8}}>📋</div><div style={{fontSize:13}}>No active programs right now. Contact your coach.</div></div>}
        ...
        {(()=>{ let globalDayIdx = 0; return visPlans.map((vp,vpIdx) => <React.Fragment key={vp.name}>
          ...
          {vp.warmup?.length > 0 && <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:14}}>
          ...
          {vp.rest && visPlans.length>1 && <div style={{background:C.sf,border:`2px solid ${C.ac}`,borderRadius:8,padding:'8px 12px',marginBottom:12,fontSize:11,color:C.tm}}>⏱ {vp.rest}</div>}
