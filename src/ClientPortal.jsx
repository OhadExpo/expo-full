...
const bi = {background:C.sf2,border:`0.25px solid ${C.ac}`,borderRadius:6,padding:"8px 10px",color:C.tx,fontFamily:FB,fontSize:14,outline:"none",width:"100%",boxSizing:"border-box",textAlign:"center"};
...
function StepLogger({day, plan, weekNum, clientId, onBack, onComplete, weeklyFocus}) {
  ...
  if (typeof step === 'string' && step.startsWith('wu')) {
    ...
    return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>{bar}
      <div style={{padding:20}}>
        ...
        {!vid && <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:30,marginBottom:14,textAlign:'center',color:C.td}}>No video for this exercise</div>}
  ...
  const renderExerciseBlock = (g, blockIdx) => {
    ...
    return <div key={ei} style={{marginBottom: blockIdx < groupExs.length - 1 ? 24 : 0, paddingBottom: blockIdx < groupExs.length - 1 ? 20 : 0, borderBottom: blockIdx < groupExs.length - 1 ? `2px dashed ${C.bd2}` : 'none'}}>
      ...
      <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:14}}>
      ...
      <div style={{padding:'14px 20px 0',display:'flex',gap:4}}>
        {[['prog','Program'],['bwt','BW Graph'],['hist',`History (${cw.length})`]].map(([k,l]) =>
          <button key={k} onClick={() => setVw(k)}
            style={{flex:1,padding:8,borderRadius:6,border:`${vw===k?'2px':'0.25px'} solid ${C.ac}`,background:vw===k?C.acD:'transparent',color:vw===k?C.ac:C.tm,fontFamily:FB,fontSize:12,fontWeight:600,cursor:'pointer',position:'relative'}}>
  ...
  if (vw === 'bwt' && trainee) {
    ...
    return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>
      ...
      <div style={{padding:'14px 20px 20px'}}>
        ...
        {/* Quick log */}
        <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:16}}>
          {visPlans.length > 1 && <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:10}}>
            {visPlans.map(p => <button key={p.name} onClick={() => setSelectedBlockName(p.name)}
              style={{padding:'4px 10px',borderRadius:14,border:`${activePlan?.name===p.name?'2px':'0.25px'} solid ${C.ac}`,background:activePlan?.name===p.name?C.acD:'transparent',color:activePlan?.name===p.name?C.ac:C.tm,fontFamily:FN,fontSize:11,fontWeight:600,cursor:'pointer'}}>{p.name}</button>)}
          </div>}
          {visPlans.length > 1 && <div style={{display:'flex',gap:4,marginBottom:10,flexWrap:'wrap'}}>
            {Array.from({length: activePlan?.weeks || 4}, (_, w) => <button key={w} onClick={() => setWk(w)} style={{flex:'1 1 40px',padding:'6px 0',borderRadius:6,border:`${wk===w?'2px':'0.25px'} solid ${C.ac}`,background:wk===w?C.acD:'transparent',color:wk===w?C.ac:C.tm,fontFamily:FN,fontSize:11,fontWeight:600,cursor:'pointer'}}>W{w+1}</button>)}
          </div>}
        ...
        {/* Graph */}
        {bwData.length < 2 ? (
          <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:40,textAlign:'center',color:C.td,marginBottom:16}}>
      ...
      {bwDeleteConfirm && <div onClick={() => setBwDeleteConfirm(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:20}}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:20,maxWidth:320,width:'100%'}}>
  ...
  if (vw === 'hist' && trainee) return <div style={{background:C.bg,color:C.tx,minHeight:'100vh',fontFamily:FB,maxWidth:500,margin:'0 auto'}}>
    ...
    <div style={{padding:'14px 20px 20px'}}>
      ...
      {cw.length === 0 ? <div style={{textAlign:'center',padding:40,color:C.td}}>No workouts yet.</div> :
        cw.slice().reverse().map(w => { const wActive = !!expandedHistEx && expandedHistEx.startsWith(w.id + ':'); return <div key={w.id} style={{background:C.sf,border:`${wActive?'2px':'0.25px'} solid ${C.ac}`,borderRadius:10,padding:12,marginBottom:8}}>
          ...
          {w.exercises.map((x,i) => {
            ...
            return (
              <div key={i} style={{marginTop:2}}>
                ...
                {isOpen && hasVideo && (
                  <div style={{marginTop:6,marginBottom:10,background:C.sf2,border:`0.25px solid ${C.ac}`,borderRadius:8,padding:8}}>
      ...
      <div style={{padding:'14px 20px 20px'}}>
        <div style={{display:'flex',gap:8,marginBottom:14,alignItems:'center'}}>
          <div style={{flex:1}}><div style={{fontSize:10,fontFamily:FN,color:C.td,marginBottom:4}}>Week</div>
            <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>{Array.from({length: activePlan?.weeks || 4}, (_, w) => <button key={w} onClick={() => setWk(w)} style={{flex:'1 1 40px',padding:'8px 0',borderRadius:6,border:`${wk===w?'2px':'0.25px'} solid ${C.ac}`,background:wk===w?C.acD:'transparent',color:wk===w?C.ac:C.tm,fontFamily:FN,fontSize:12,fontWeight:600,cursor:'pointer'}}>W{w+1}</button>)}</div></div>
          <div style={{width:120}}><div style={{fontSize:10,fontFamily:FN,color:C.td,marginBottom:4}}>BW {lb?`(${lb}kg)`:''}</div>
            <div style={{display:'flex',gap:4}}>
            <input value={bw} onChange={e => setBw(e.target.value)} placeholder="kg" type="number" disabled={!activePlan} style={{background:C.sf2,border:`0.25px solid ${C.ac}`,borderRadius:6,padding:'8px',color:C.tx,fontFamily:FN,fontSize:12,outline:'none',width:'100%',boxSizing:'border-box',textAlign:'center',opacity:activePlan?1:0.5}}/>
        ...
        {activePlan?.rest && <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:10,padding:'10px 14px',marginBottom:14,fontSize:12,color:C.tm}}>⏱ {activePlan.rest}</div>}
        ...
        {visPlans.length===0 && !plansLoadError && <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:30,textAlign:'center',color:C.td,marginBottom:14}}><div style={{fontSize:20,marginBottom:8}}>📋</div><div style={{fontSize:13}}>No active programs right now. Contact your coach.</div></div>}
        ...
        {(()=>{ let globalDayIdx = 0; return visPlans.map((vp,vpIdx) => <React.Fragment key={vp.name}>
          ...
          {vp.warmup?.length > 0 && <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:12,padding:14,marginBottom:14}}>
          ...
          {vp.rest && visPlans.length>1 && <div style={{background:C.sf,border:`0.25px solid ${C.ac}`,borderRadius:8,padding:'8px 12px',marginBottom:12,fontSize:11,color:C.tm}}>⏱ {vp.rest}</div>}
